...
| Value | Behavior |
| --- | --- |
| `minimal` | Default. Exposes `open_thread`, `read`, `write`, `edit`, and `bash`. Clients use `bash` with tools such as `rg`, `find`, and `ls` for inspection. |
| `full` | Exposes the minimal tools plus dedicated `grep`, `glob`, and `ls` tools. |
| `codex` | Experimental. Exposes `open_thread`, `read`, `apply_patch`, `exec_command`, and `write_stdin`. Existing mutation and shell tools are hidden. |
...
| `changes` | Enables the aggregate `show_changes` tool and attaches widget UI to `open_thread` and `show_changes`. |
