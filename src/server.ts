...
function serverInstructions(config: ServerConfig): string {
  ...
  return `Use DevSpace for coding work. Call ${toolNames.openWorkspace} once for each project folder or isolated worktree, then keep using its workspaceId. During continued work in the same project or worktree, do not call ${toolNames.openWorkspace} again. Open another workspace only when changing projects, switching checkout/worktree mode, creating another isolated worktree, or when the current workspaceId is rejected. ${agentsMd}${skills}${inspection}Prefer ${toolNames.edit} for targeted modifications, ${toolNames.write} only for new files or complete rewrites, and ${toolNames.shell} for tests, builds, package scripts, routine Git commands, and commands that are better executed by the shell. Git read operations such as git status and git diff, plus the Git write operations git add, git commit, git fetch, git pull, and git push, are allowed. This guidance does not authorize destructive Git commands such as git reset --hard, git clean, git checkout --, or branch deletion. Outside the explicitly allowed Git operations, do not create or modify project source files with ${toolNames.shell}; avoid shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, or any command whose purpose is to write project source files.${artifactInstruction}${showChangesInstruction}`;
}
  ...
  registerAppTool(
    ...
    toolNames.shell,
    {
      ...
      description: config.toolMode !== "full"
        ? `Run a shell command in a workspace for tests, builds, package scripts, routine Git commands, search, file discovery, directory inspection, and other shell-side tooling. Git read operations such as git status and git diff, plus the Git write operations git add, git commit, git fetch, git pull, and git push, are allowed. This guidance does not authorize destructive Git commands such as git reset --hard, git clean, git checkout --, or branch deletion. In minimal tool mode, ${toolNames.grep}, ${toolNames.glob}, and ${toolNames.ls} are disabled; use command-line tools such as grep, rg, find, ls, and tree for those read-only inspection actions. Outside the explicitly allowed Git operations, do not use ${toolNames.shell} to create or modify project source files. Do not use shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, or generated scripts to write project source files; use ${toolNames.edit} for targeted changes and ${toolNames.write} for new files or full rewrites. Prefer ${toolNames.read} for direct file reads. This is powerful execution and should only be exposed behind strong authentication.`
        : `Run a shell command in a workspace for tests, builds, package scripts, routine Git commands, and commands that are better executed by the shell. Git read operations such as git status and git diff, plus the Git write operations git add, git commit, git fetch, git pull, and git push, are allowed. This guidance does not authorize destructive Git commands such as git reset --hard, git clean, git checkout --, or branch deletion. Outside the explicitly allowed Git operations, do not use ${toolNames.shell} to create or modify project source files. Do not use shell redirection, heredocs, tee, sed -i, perl -i, node/python/ruby scripts, or generated scripts to write project source files; use ${toolNames.edit} for targeted changes and ${toolNames.write} for new files or full rewrites. Prefer ${toolNames.read}, ${toolNames.grep}, ${toolNames.glob}, and ${toolNames.ls} for file inspection. This is powerful execution and should only be exposed behind strong authentication.`,
      inputSchema: {
        workspaceId: z
          .string()
          .describe(workspaceIdDescription),
        command: z
          .string()
          .describe(
            `Shell command to run. Git read operations such as git status and git diff, plus the Git write operations git add, git commit, git fetch, git pull, and git push, are allowed. This guidance does not authorize destructive Git commands such as git reset --hard, git clean, git checkout --, or branch deletion. Outside the explicitly allowed Git operations, must not create or modify project source files; use ${toolNames.edit} or ${toolNames.write} for source file changes.`,
          ),
        workingDirectory: z
          .string()
          .optional()
