...
const fixtureRoot = await mkdtemp(join(tmpdir(), "devspace-patch-fixtures-"));
try {
  const root = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-"));
...
*** End Patch`,
  ...
  const outside = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-outside-"));
...
*** End Patch`,
  ...
  const splitHunkRoot = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-split-hunk-"));
  await writeFile(
...
*** End Patch`,
  ...
  const trailingSpaceRoot = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-trailing-space-"));
...
*** End Patch`,
  ...
  const overwriteRoot = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-overwrite-"));
...
*** End Patch`,
  ...
  const noNewlineRoot = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-newline-"));
...
*** End Patch`,
  ...
  const eofRoot = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-eof-"));
...
*** End Patch`,
  ...
  const lenientRoot = await mkdtemp(join(fixtureRoot, "devspace-apply-patch-lenient-"));
