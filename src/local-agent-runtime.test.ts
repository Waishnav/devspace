...
test("provider connection errors preserve retryability", async (t) => {
...
test("concurrent runs share a runtime that stays alive until shutdown", async (t) => {
...
test("session reuse waits for release to finish", async (t) => {
  let clock = 0;
  ...
  sessionRuntime.finishSessionRelease();
  await releasing;
  assert.equal(unwrap(await reused).finalResponse, "done:reuse");
...
test("shutdown waits for release and rejects queued reuse", async (t) => {
  ...
  t.after(async () => {
    ...
    shutdownReleaseRuntime.releaseWait();
    await shutdownReleasePool.close();
  });
...
test("cleanup errors preserve the original provider failure", async (t) => {
  ...
  t.after(async () => {
    cleanupRuntime.finishSessionRelease();
    cleanupRuntime.releaseWait();
...
test("a dead runtime is replaced despite cleanup failure", async (t) => {
  ...
  t.after(async () => {
    replacementRuntime.finishSessionRelease();
    replacementRuntime.releaseWait();
...
test("shutdown preserves an already completed turn", async (t) => {
  ...
  t.after(async () => {
    completedTurnRuntime.finishSessionRelease();
    completedTurnRuntime.releaseWait();
    await completedTurnPool.close();
  });
...
test("shutdown closes a runtime created while stopping", async (t) => {
...
test("failed concurrent creation can recover on a later run", async (t) => {
  ...
  t.after(async () => {
    recoveryRuntime.finishSessionRelease();
    recoveryRuntime.releaseWait();
  ...
  const recovered = unwrap(await creationPool.run(creationDriver, context, input));
  assert.equal(recovered.finalResponse, "done:inspect");
  assert.equal(createAttempts, 2, "a later caller can retry after creation fails");
  await creationPool.close();
});

test("failed session release remains reusable and retries later", async (t) => {
  ...
  t.after(async () => {
    releaseRuntime.finishSessionRelease();
    releaseRuntime.releaseWait();
